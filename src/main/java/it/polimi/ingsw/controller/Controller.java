    ...
    private void grabbingAction(ClientHandler clientHandler, String msg){
        try {
            ...
            try {
                ...
                Weapon weapon = ((SpawnPoint) match.getBoard().find(Integer.parseInt(msg.substring(ETIQUETTE).split("-")[0].split(",")[0]), Integer.parseInt(msg.substring(ETIQUETTE).split("-")[0].split(",")[1]))).getWeapons().get(Integer.parseInt(msg.substring(ETIQUETTE).split(",")[2]));
                playerFromNickname(clientHandler.getName(), this.match).pay(
